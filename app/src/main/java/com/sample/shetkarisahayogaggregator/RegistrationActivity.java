...
public class RegistrationActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    TextView regGender;
    EditText regName, regMiddleName, regMobileNumber, regDob, regAddress, regPincode, regBusinessType, regBankAccNumber, regIFSCCode;
    EditText doc1, doc2, doc3, regAadharCard, regPanCard;
    EditText regCountry, regState, regDistrict, regCity, regDemandingCrops;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        regBusinessType = findViewById(R.id.editText_businessType);
        regDemandingCrops = findViewById(R.id.editText_demandingCrops);

        doc1 = findViewById(R.id.editText_businessDocs1);
        doc2 = findViewById(R.id.editText_businessDocs2);
        doc3 = findViewById(R.id.editText_businessDocs3);
        ...
        findViewById(R.id.imageView_businessDocsUpload1).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        findViewById(R.id.imageView_businessDocsUpload2).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
                startActivityForResult(intent, 23);
        ...
        findViewById(R.id.imageView_businessDocsUpload3).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
                startActivityForResult(intent, 23);
    ...
    private Boolean validateBusinessType() {
        String val = regBusinessType.getText().toString();
        if (val.isEmpty()) {
            regBusinessType.setError("Field cannot be empty");
            return false;
        } else {
            regBusinessType.setError(null);
            return true;
        }
    }

    private Boolean validatedemandingCrops() {
        String val = regDemandingCrops.getText().toString();
        if (val.isEmpty()) {
            regDemandingCrops.setError("Field cannot be empty");
            return false;
        } else {
            regDemandingCrops.setError(null);
            return true;
        }
    }
    ...
    public void onNextButton(View view) {
        if (!validateName() | !validateMiddleName() | !validateGender() | !validateDob() | !validateMobileNumber() | !validateAddress() | !validateCountry() | !validateState() | !validateDistrict() | !validateCity() | !validatePincode() | !validateBusinessType() | !validatedemandingCrops() | !validateBankAccountNumber() | !validateIFSCCode()) {
            //Do nothing
        } else {
            ...
            String businessType = regBusinessType.getText().toString();
            String demandingCrops = regDemandingCrops.getText().toString();
            ...
            UserHelperClass helperClass = new UserHelperClass(name, middleName, gender, dob, mobileNumber, address, country, state, district, city, pincode, businessType, demandingCrops, bankAccNumber, ifscCode, id);
            FirebaseDatabase.getInstance().getReference("Aggregator").child(mobileNumber).setValue(helperClass);
